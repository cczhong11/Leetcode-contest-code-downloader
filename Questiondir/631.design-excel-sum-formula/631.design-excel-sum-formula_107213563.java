	    ...
	    public int sum(int r, char cc, String[] strs) {
	    	...
	    	for(String s : strs){
	    		...
	    		if(colon >= 0){
	    			...
	    			for(int j = Math.min(one[0], two[0]);
	    					j <= Math.max(one[0], two[0]);j++){
	    				for(int k = Math.min(one[1], two[1]);
	    						k <= Math.max(one[1], two[1]);k++){
	    					g[j*26+k][x]++;
	    				}
	    			}
	    		}else{
